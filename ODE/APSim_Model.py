...
def vectorfield(w, t, p, params ):
    ...
    ACH, ND_N, TGFB,PDGF, FIBRO, MMP, CM9, COLL  = pre_vectorfield(w, params)
    ...
    step_max, r_NDN, r_ITM, ITMblood_peak, r_AP, rinduce, ITM_source_peak, ITM_source_width, mu_TGFB, beta_TGFB_M_A, \
    beta_TGFB_FIBRO, beta_M_PDGF, beta_F_PDGF, mu_PDGF, mu_FIBRO, r_F, beta_M_MMP, Ti, \
    k_off, k_on, mu_MMP, k_onc, mu_COLL, beta_FIBRO_COLL, MAX_FIBRO, MAX_COLL, MAX_M_A,  = p
    ...
    f_dillution = 1./16. # (80/5) ratio between blood tissue compartment and blood compartment
    ...
    f = [
        ...
        * (int(not (ITMblood < 0)))
        ...
        #TGFB
        - mu_TGFB * TGFB + M_A * beta_TGFB_M_A + beta_TGFB_FIBRO * FIBRO,

        #PDGF
        + M_A * beta_M_PDGF + FIBRO * beta_F_PDGF - mu_PDGF * PDGF,

        # FIBROBLASTS
        - mu_FIBRO * FIBRO + r_F * (1-crowding) * FIBRO + prms.fibroblast_vs_PDGF(PDGF) ,

        #M9A
        + beta_M_MMP * M_A * (1/(1+TGFB/Ti)) +k_off * CM9 - k_on * MMP * COLL - mu_MMP * MMP,

        #CM9
        + k_on * MMP * COLL - k_onc * CM9,

        #COLLAGEN
         beta_FIBRO_COLL * FIBRO * prms.collagen_vs_TGFB(TGFB) * prms.collagen_vs_collagen_density(COLL) +k_off * CM9 - k_on * MMP * COLL,
...
def pre_vectorfield(w, params):
    N_R, AP_Eblood, AP_Etissue, AP_Eliver, AP_Sblood, AP_Stissue, ITMb, ITM, M_R, M_A, CH, N_A, ND_A, ACH, ND_N, TGFB,PDGF, FIBRO, MMP,CM9, COLL = w
    if params['case'] == 0:
        N_R = AP_Eblood = AP_Etissue, AP_Eliver = AP_Sblood = AP_Stissue = M_R = M_A = CH = N_A = ND_A = ACH = ND_N = TGFB= PDGF = FIBRO= MMP= CM9 = COLL= 0
    elif params['case'] == 1:
    ...
    return [N_R, AP_Eblood, AP_Etissue, AP_Eliver, AP_Sblood, AP_Stissue, ITMb, ITM, M_R, M_A, CH, N_A, ND_A, ACH, ND_N,TGFB,PDGF, FIBRO, MMP,CM9, COLL]


def post_vectorfield(f, w, params):
    _w = place_finder(w)
    N_R, AP_Eblood, AP_Etissue, AP_Eliver, AP_Sblood, AP_Stissue, ITMb, ITM, M_R, M_A, CH, N_A, ND_A, ACH, ND_N, TGFB, PDGF, FIBRO, MMP,CM9, COLL= _w
    if params['case'] == 0:
        f[N_R] = f[AP_Eblood] = f[AP_Etissue] = f[AP_Eliver] = f[AP_Sblood] = f[AP_Stissue] \
            = f[ITMb] = f[ITM] = f[M_R] = f[M_A] = f[CH] = f[N_A] = f[ND_A] = f[ACH] = f[ND_N] = f[TGFB] = f[PDGF] =f[FIBRO] = f[MMP] = f[CM9] =  f[COLL] =0
    elif params['case'] == 1:
...
def get_init(w0, params):
    ...
    Arguments:
        w0       :  vector of the initial conditions
    """
    N_R0, AP_Eb0, AP_Et0, AP_El0, AP_S0, AP_St0, ITMb0, ITM0, M_R0, M_A0, CH0, N_A0, ND_A0, ACH0, ND_N0, TGFB0, PDGF0,  FIBRO0, MMP0,CM90, COLL0= w0

    if params['case'] == 0:
        N_R0 = AP_Eb0 = AP_Et0 = AP_El0 = AP_S0 = AP_St0 = M_R0 = M_A0 = CH0 = N_A0 = ND_A0 = ACH0 = ND_N0 = TGFB0 = PDGF0 = FIBRO0 = MMP0 = CM90, COLL0= 0
    elif params['case'] == 1:
        ...
        AP_S0 = AP_St0 = 0
    w0 = [N_R0, AP_Eb0, AP_Et0, AP_El0, AP_S0, AP_St0, ITMb0, ITM0, M_R0, M_A0, CH0, N_A0, ND_A0, ACH0, ND_N0,TGFB0, PDGF0, FIBRO0, MMP0, CM90, COLL0]
    return w0
...
def get_header():
    header = ['N_R', 'AP_Eblood', 'AP_Etissue', 'AP_Eliver', 'AP_Sblood', 'AP_Stissue', 'ITMblood', 'ITMtissue',
              'M_R', 'M_A', 'CH', 'N_A', 'ND_A', 'ACH', 'ND_N','TGFB', 'PDGF', 'FIBRO', 'MMP','CM9', 'COLL' ]
    return header
...
def plot(wsol, df_AP_median, case_names, timein, t_interval, df_IL6, df_IL10, params):
    ...
    tfile = open('infection/antiinflammation_change/test5.csv', 'w')
    tfile.write(df_model.to_csv())
    tfile.close()

    for i in df_model['COLL']:
        WOUND_SIZE.append([max((1-i/1200),0)])


    if case >= 1:
        n = loop_fig(5)
        plt.figure(n)
        plt.plot(t, df_model['FIBRO'], linewidth=lw, label=case_label, ls='-', color= color)
        plt.xlabel("Hours After Insult", fontsize=fs)
        plt.ylabel("Fibroblasts $(cells/mm^3$)", fontsize=fs)
        plt.title("Fibroblasts ", fontsize=ts)
        plt.legend(loc='best', fontsize=lfs)
        plt.xlim((0, prms._stoptime / 60.))
        plt.tick_params(labelsize=ls)
        print('... Plotting FIBRO')
        plt.savefig('result/plots/' + 'FIBRO.png', format='png', dpi=500, bbox_inches='tight')

        n = loop_fig(n)
        plt.figure(n)
        plt.plot(t, df_model['PDGF'], linewidth=lw, label=case_label, ls='-', color=color)
        plt.xlabel("Hours After Insult", fontsize=fs)
        plt.ylabel("concentration $(pg/mm^3)$", fontsize=fs)
        plt.title("PDGF", fontsize=ts)
        plt.legend(loc='best', fontsize=lfs)
        plt.xlim((0, prms._stoptime / 60.))
        plt.tick_params(labelsize=ls)
        print('... Plotting PDGF')
        ...
        n = loop_fig(n)
        plt.figure(n)
        plt.plot(t, df_model['TGFB'], linewidth=lw,label=case_label, ls='-', color=color)
        plt.xlabel("Hours After Insult", fontsize=fs)
        plt.ylabel("concentration $(pg/mm^3)$", fontsize=fs)
        ...
        plt.tick_params(labelsize=ls)
        print('... Plotting TGFB')
        plt.savefig('result/plots/' + 'TGFB.png', format='png', dpi=500, bbox_inches='tight')

        n = loop_fig(n)
        plt.figure(n)
        plt.plot(t, df_model['COLL'], linewidth=lw, label=case_label, ls='-', color=color)
        plt.xlabel("Hours After Insult", fontsize=fs)
        plt.ylabel("concentration $(\u03bcg/mm^3$)", fontsize=fs)
        plt.title("Collagen", fontsize=ts)
        plt.legend(loc='best', fontsize=lfs)
        plt.xlim((0, prms._stoptime / 60.))
        plt.tick_params(labelsize=ls)
        print('... Plotting Collagen')
        plt.savefig('result/plots/' + 'COLL.png', format='png', dpi=500, bbox_inches='tight')

        n = loop_fig(n)
        plt.figure(n)
        plt.plot(t, df_model['MMP'], linewidth=lw, label=case_label, ls='-', color=color)
        plt.xlabel("Hours After Insult", fontsize=fs)
        plt.ylabel("concentration $(\u03bcg/mm^3$)", fontsize=fs)
        plt.title("Actived MMP-9", fontsize=ts)
        plt.legend(loc='best', fontsize=lfs)
        plt.xlim((0, prms._stoptime / 60.))
        plt.tick_params(labelsize=ls)
        print('... Plotting MMP')
...
if __name__ == "__main__":
    ...
    mu_TGFB = prms.mu_TGFB
    mu_FIBRO = prms.mu_FIBRO
    mu_COLL = prms.mu_COLL
    mu_PDGF = prms.mu_PDGF
    mu_MMP = prms.mu_MMP


    beta_M_PDGF = prms.beta_M_PDGF
    beta_F_PDGF = prms.beta_F_PDGF
    beta_TGFB_FIBRO = prms.beta_TGFB_FIBRO
    beta_TGFB_M_A = prms.beta_TGFB_M_A
    beta_M_MMP = prms.beta_M_MMP
    beta_FIBRO_COLL = prms.beta_FIBRO_COLL

    r_F = prms.r_F
    Ti = prms.Ti

    MAX_FIBRO = prms.MAX_FIBRO
    MAX_COLL = prms.MAX_COLL
    MAX_M_A = prms.MAX_M_A
    ...
    for case in cases:
        ...
        w = [N_R0, AP_Eb0, AP_Et0, AP_El0, AP_S0, AP_St0, ITMblood0, ITMtissue0, M_R0, M_A0, CH0, N_A0, ND_A0, ACH0, ND_N0, TGFB0, PDGF0, FIBRO0, MMP0,CM90, COLL0]
        params['case'] = case
